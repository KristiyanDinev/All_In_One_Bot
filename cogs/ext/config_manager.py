...
class ConfigManager:
    def __init__(self, file_path, messages_path, warnings_path):
    ...
    def getCommandData(self, command_name):
        try:
            return self.messages.get("commands").get(command_name)
        except Exception:
            print("You miss-configure the command " + command_name)
            exit()

    def getCommandArgs(self, command_name, argument):
        res = self.getCommandData(command_name).get("args", {}).get(argument, None)
        if res is None:
            return self.messages.get("args", {}).get(argument, None)

    def getCommandMessage(self, command_name, message_key):
        res = self.getCommandData(command_name).get("messages", {}).get(message_key, None)
        if res is None:
            return self.messages.get("messages", {}).get(message_key, None)

    def getInvalidArgsKey(self):
        #return self.messages.get("invalid_args")
        return "invalid_args"

    def getInvalidMemberKey(self):
        #return self.messages.get("invalid_member")
    ...
    def getInvalidRoleKey(self):
        #return self.messages.get("invalid_role")
        return "invalid_role"

    def getAddedRoleKey(self):
        #return self.messages.get("add_role")
        return "add_role"
    ...
    def getEmbeds(self):
        return self.messages.get("embed_format")
    ...
    def isActivePlaceholder(self, placeholder: str):
        return placeholder in self.messages.get("activated_placeholders")
